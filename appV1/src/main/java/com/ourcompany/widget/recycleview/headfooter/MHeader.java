package com.ourcompany.widget.recycleview.headfooter;
...
public class MHeader extends InternalClassics<MHeader> implements RefreshHeader {
...
//    public static String REFRESH_HEADER_UPDATE = "'Last update' M-d HH:mm";
    ...
    public MHeader(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MHeader(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        lpArrow.width = (int) (ta.getLayoutDimension(R.styleable.ClassicsHeader_srlDrawableArrowSize, lpArrow.width)*1.3);
        lpArrow.height = (int) (ta.getLayoutDimension(R.styleable.ClassicsHeader_srlDrawableArrowSize, lpArrow.height)*1.3);
        lpProgress.width = (int) (ta.getLayoutDimension(R.styleable.ClassicsHeader_srlDrawableProgressSize, lpProgress.width)*0.8);
        lpProgress.height = (int) (ta.getLayoutDimension(R.styleable.ClassicsHeader_srlDrawableProgressSize, lpProgress.height)*0.8);
...
//
//        }
        mArrowView.setImageDrawable(ResourceUtils.getDrawable(R.drawable.ic_arrow));
...
//    }
    ...
    public MHeader setLastUpdateTime(Date time) {
        final View thisView = this;
        mLastTime = time;
    ...
    public MHeader setTimeFormat(DateFormat format) {
    ...
    public MHeader setLastUpdateText(CharSequence text) {
        mLastTime = null;
        mLastUpdateText.setText(text);
        return this;
    }

    public MHeader setAccentColor(@ColorInt int accentColor) {
        mLastUpdateText.setTextColor(accentColor & 0x00ffffff | 0xcc000000);
        return super.setAccentColor(accentColor);
    }

    public MHeader setEnableLastTime(boolean enable) {
        final View updateView = mLastUpdateText;
...
//            mRefreshKernel.requestRemeasureHeightForHeader();
    ...
    public MHeader setTextSizeTime(float size) {
...
//    }
    ...
    public MHeader setTextTimeMarginTop(float dp) {
        final View updateView = mLastUpdateText;
