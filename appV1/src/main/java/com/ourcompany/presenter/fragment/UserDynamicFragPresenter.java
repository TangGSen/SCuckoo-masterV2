...
public class UserDynamicFragPresenter extends MvpBasePresenter<FoundNewsFragmentView> {
    BmobDate bmobDate;

    public UserDynamicFragPresenter(Context context) {
        super(context);
    }
    ...
    public void getData(final int start, final boolean isLoadMore, String blongUserId) {
        if (TextUtils.isEmpty(blongUserId)) {
            showEmptyView();
            return;
        }
        ...
        query2.addWhereEqualTo(Constant.BMOB_SUSER_ID, blongUserId);
        queries.add(query2);
        //
        BmobQuery<Post> queryRes = new BmobQuery<Post>();
        queryRes.include(Constant.BMOB_POST_USER);
        queryRes.order(Constant.BMOB_ORDER_DESCENDING + Constant.BMOB_CREATE);
        queryRes.setLimit(Constant.IM_PAGESIZE);
        queryRes.setSkip(start * Constant.IM_PAGESIZE);
        queryRes.and(queries);
        //执行查询方法
        queryRes.findObjects(new FindListener<Post>() {
            @Override
    ...
    public void getDataOnLoadMore(int currentIndex, String blongUserId) {
        getData(currentIndex, true, blongUserId);
    }
}
